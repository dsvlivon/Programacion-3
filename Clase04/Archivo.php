...
class Archivo{
    ...
    static function GuardarJSON($archivo, $line){              
        if($line!=null){
            if (!file_exists($archivo)){
                $p = fopen($archivo, "w");
                $ret = fwrite($p, "[".$line."]");
                ...
                $ret = fwrite($p, ",\n".$line."]");//ojo c ese caracter d escape
            }
            fclose($p);              
            return $ret;     
        }
    }
    static function GuardarJSONLista($line,$archivo){      
        if($line!=null){
            $p = fopen($archivo, "w");
            $ret = fwrite($p, json_encode($line));
            fclose($p);    
            return $ret;
        } else {
    ...
    private function __construct() {
        try {//ACA SIEMPRE ES IMPORTANTE EDITAR EL NOMBRE DE LA DB (no confundir c nombre de tabla)!!
            ...
            $this->objetoPDO = new PDO('mysql:host=localhost;dbname='.$db.';charset=utf8', 'root', '', array(PDO::ATTR_EMULATE_PREPARES => false,PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
